    ...
    <div className="container mx-auto max-w-2xl px-4 py-6">
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>
            Enter their email (must match the account they will use to accept). Choose what they can see — they cannot
            edit your expenses or lists.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={onInvite} className="space-y-4">
            ...
            <fieldset className="space-y-2">
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                My Expenses (bills, paid status, read-only)
              </label>
              <label className="flex items-center gap-2 text-sm">
                ...
                {s.status === "pending" && (
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                    <code className="block max-w-full truncate rounded bg-muted px-2 py-1 text-xs">
                      {`${base}/settings/sharing/accept?share=${s.id}&token=${s.invite_token}`}
                    </code>
                    <Button
                      type="button"
                      size="sm"
