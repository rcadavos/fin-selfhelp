...
export default async function SharedHubPage({ params }: Props) {
  ...
  if (!user) redirect(`/login?next=${encodeURIComponent(`/shared/${grantorUserId}`)}`);

  const { data: grantorProfile } = await supabase.from("profiles").select("id").eq("user_id", grantorUserId).single();
  if (!grantorProfile) redirect("/shared");
        ...
        <CardContent className="flex flex-col gap-2">
          {share.can_view_expenses && (
            <Button asChild variant="outline">
              <Link href={`/shared/${grantorUserId}/my-expenses`}>My Expenses</Link>
            </Button>
          )}
          {share.can_view_to_buy && (
            <Button asChild variant="outline">
              <Link href={`/shared/${grantorUserId}/to-buy`}>To-buy list</Link>
            </Button>
          )}
          <Button asChild variant="ghost" className="mt-2">
            <Link href="/shared">All shared accounts</Link>
          </Button>
        </CardContent>
      </Card>
    </div>
