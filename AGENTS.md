...
9. **Shared UI patterns (app-wide updates):** If the same control, layout, or interaction appears in **more than one** route or feature (filter dropdowns, dialog footers, empty states, repeated toolbars, etc.), **implement it once**—typically under `@/components/ui/*`, or in a shared feature folder such as `@/components/app/*` when it is app-shell-specific—and **compose it with props** for labels, icons, or options. Do not copy-paste equivalent JSX across pages to save time; that causes inconsistent behavior and styling. When the user asks to change how such a pattern looks or behaves, **change the shared component** so the update applies **everywhere it is used**, unless they explicitly narrow the request to one screen—in that case extend the shared component with props or variants instead of duplicating markup.

## UI & styling

1. **Reusable components:** Prefer `@/components/ui/*` (Button, Card, Input, Skeleton, etc.) and existing shared components. Follow **Convention 9**: one shared implementation per repeated pattern so a single edit can roll out app-wide; use props for intentional differences between screens.
